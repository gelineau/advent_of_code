...
def star(filename: str):
    ...
    for row in range(len(input)):
        ...
        starts.append(((row, len(input[0]) - 1), left))

    for column in range(len(input[0])):
        starts.append(((0, column), down))
        starts.append(((len(input) - 1, column), up))

    for i, start in enumerate(starts):
        print(f"{i=} {len(starts)=} {max_length=}")
        states = [start]
        max_length = max(max_length, calculate_length(input, states))
    return max_length
