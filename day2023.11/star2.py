...
def shortest_path(galaxy1, galaxy2, empty_rows, empty_columns):
    ...
    delta = row2 - row1 + expension * sum(1 for row in empty_rows if row1 < row < row2)
    delta += (
        column2
        - column1
        + expension * sum(1 for column in empty_columns if column1 < column < column2)
    )
